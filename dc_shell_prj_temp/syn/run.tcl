...
# -------------------------------------------------------------------------------
# This is the template Design Compiler script for experiment
################################################################################
...
# - link_library
################################################################################
set_app_var search_path "$search_path $my_lib_path ../rtl"
set my_lib_path "/opt/Foundary_Library/TSMC90/aci/sc-x/synopsys"

set_app_var synthetic_library {fast.db fast_leakage.db fastz.db slow.db typical.db typical_leakage.db}
set_app_var target_library {fast.db fast_leakage.db fastz.db slow.db typical.db typical_leakage.db}
set_app_var link_library {* fast.db fast_leakage.db fastz.db slow.db typical.db typical_leakage.db}
...
#Add all the source files in ./rtl folder here
analyze -format verilog {BCD_encoder.v} 
#Add top module's name here
elaborate BCD_encoder; 

# store the unmapped results, the umapped results name is defined by customed
write -hierarchy -format ddc -output results/BCD_encoder.unmapped.ddc
...
source BCD_encoder.constraints.tcl
...
check_design > reports/BCD_encoder.check_design.rpt; # dump to the file
...
# Write out design
write -format verilog -hierarchy -output results/BCD_encoder.mapped.v
write -format ddc -hierarchy -output results/BCD_encoder.mapped.ddc
write_sdf results/BCD_encoder.mapped.sdf
write_sdc -nosplit results/BCD_encoder.mapped.sdc

# Generate reports
report_qor > reports/BCD_encoder.mapped.qor.rpt
report_timing -transition_time -nets -attribute -nosplit \
  > reports/BCD_encoder.mapped.timing.rpt
report_area -nosplit > reports/BCD_encoder.mapped.area.rpt
report_power -nosplit > reports/BCD_encoder.mapped.power.rpt
